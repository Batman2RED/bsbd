<?php
	...
	{	
		...
		{	
			...
			$sql = 'SELECT acc_id FROM lib_session WHERE session_id = :sess_id';
			$stmt = $db->prepare($sql);
			...
			{	
				...
				$sess_update = 'UPDATE lib_session SET session_date = :s_date, session_id = :s_id WHERE acc_id = :acc_id';
	...
	function mySession_write($acc_id)
	{
		...
		$sess_check = 'SELECT acc_id FROM lib_session WHERE acc_id = :account_id';
		...
		if ($acc)
		{
			$sess_update = 'UPDATE lib_session SET session_date = :s_date, session_id = :s_id';
		...
		$sql = 'INSERT INTO lib_session(session_id, session_date, acc_id) VALUES (:session_id, :session_date, :acc_id)';
	...
	function mySession_stop()
	{
		...
		$sql = 'DELETE FROM lib_session WHERE session_id = :sess_id';
		$stmt = $db->prepare($sql);
